...
// On-demand owner skip-trace (paid, per-hit via Tracerfy). Fired only when the
// user explicitly asks — never automatically from /api/report — so no one is
// billed for a lookup they didn't request. If TRACERFY_API_KEY isn't set, the
// endpoint reports the feature is off rather than erroring.
